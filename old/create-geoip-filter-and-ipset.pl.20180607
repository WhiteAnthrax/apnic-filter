...
foreach $country (keys %countries) {
	...
	print $fh "ipset flush $country\n";
	print $fh "ipset create -exist $country hash:net\n";
    foreach $line (@aggregated) {
		...
		print $fh "ipset add $country $line\n";
...
print "please run 'sudo /bin/sh $dirname/data/$year/$date'\n";
print "*** 完了\n";
